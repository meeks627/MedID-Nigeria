...
if (GEMINI_API_KEY && GEMINI_API_KEY !== "MY_GEMINI_API_KEY") {
  ...
  console.log("Gemini API Key missing or default placeholder. AI features will run in high-fidelity simulated backup mode.");
}

// ==========================================
// IN-MEMORY SIMULATED EHR DATABASES
// ==========================================
// Each hospital maintains its own local records. MedID only indexes.
interface Encounter {
  date: string;
  doctorName: string;
  department: string;
...
// ==========================================
// MEDID SHARED REGISTRY (MedID Core Platform)
// ==========================================
// In-memory simulation of patient registry, doctors, emergency codes, and audit logs
...
const REGISTRY_HOSPITALS: HospitalProfile[] = [
  {
    ...
    codeGeneratedAt: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000), // 2 days ago
  },
  {
    id: "LASUTH",
    ...
    codeGeneratedAt: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000), // 5 days ago
  },
  {
    id: "Evercare",
    ...
    codeGeneratedAt: new Date(Date.now() - 10 * 24 * 60 * 60 * 1000), // 10 days ago
  },
];
...
app.post("/api/patient/register", (req, res) => {
  ...
  // Generate unique MedID starting with MD followed by 8 unique digits
  ...
  const newPatient: PatientProfile = {
    ...
    linkedHospitals: [], // Initially empty, will adapt as visits occur
  };

  REGISTRY_PATIENTS.push(newPatient);

  // Return generated MedID and simulated actions
  res.json({
    success: true,
    medID,
    patient: newPatient,
...
app.post("/api/patient/login", (req, res) => {
  ...
  // Return profile and custom access history logs ONLY
...
app.post("/api/admin/login", (req, res) => {
  ...
  // Simple hardcoded access key check for development convenience
...
// --- HOSPITAL ADMIN ACTIONS: DOCTORS ---
...
// --- HOSPITAL ADMIN ACTIONS: EMERGENCY OVERRIDE ---
...
// --- HOSPITAL ADMIN VIEW VISITED PATIENTS ---
app.get("/api/admin/:hospitalId/patients", (req, res) => {
  const { hospitalId } = req.params;
  // Look at patient profiles that are linked to this hospital or have encounters in our mock database
...
// --- HOSPITAL ADMIN VIEW AUDIT LOGS ---
app.get("/api/admin/:hospitalId/logs", (req, res) => {
  ...
  // Filter logs associated with this hospital name
...
app.get("/api/doctor/search-patient", (req, res) => {
  ...
  // Scan simulated EHRs to check where records exist
  ...
  // Return ONLY patient name, medID, and record checkmarks (No clinical information!)
  res.json({
    name: patient.name,
    medID: patient.medID,
    dob: patient.dob,
...
// --- NORMAL RETRIEVAL & CONSENT ---
// Under Patient Spec: Typing MedID itself represents patient consent.
// No Request Access or Approve Access is needed.
app.post("/api/doctor/retrieve-records", (req, res) => {
  ...
  // Fetch full records dynamically via Hospital Adapters (simulated call)
  ...
  // Auto link hospital to patient if not already linked
...
// --- EMERGENCY OVERRIDE PATIENT RECOVERY ---
app.post("/api/doctor/emergency-biometric-match", (req, res) => {
  const { biometricType, scanData, reason, doctorId } = req.body;
  // Biometric simulation maps to NIN holder
  // Let's assume the fingerprint, face, or palm scan matches Sarah Johnson for test reliability,
  // but allow choosing David or Chioma based on simulated scan selection!
  let selectedPatient = REGISTRY_PATIENTS[0]; // Default Sarah Johnson
...
app.post("/api/doctor/emergency-retrieve", (req, res) => {
  ...
  // Validate the emergency override code
  ...
  // Retrieve records from all hospitals for emergency safety
  const retrievedRecords: { [hospitalName: string]: Encounter[] } = {};
  Object.keys(EHR_DATABASES).forEach((hospId) => {
    const hospRecord = EHR_DATABASES[hospId].patients[medID];
    if (hospRecord) {
...
// ==========================================
// GEMINI CLINICAL ASSISTANCE CORES
// ==========================================

// Helper to format medical records into standard text for Gemini context
...
Never include mock system jargon like "STATUS: LIVE" or credit lines. Use professional medical terminology.`;
  ...
  if (ai) {
    try {
      ...
      // Fallback to high-fidelity local parser in case of rate-limiting or network error
    }
  }

  // High-fidelity local parser backup (works seamlessly offline or if key is omitted)
...
Rule 4: Keep your answer highly professional, clinical, and directly relevant to the patient's care.`;
  ...
  if (ai) {
    try {
      // Map user messages to Gemini content format
...
// ==========================================
// SERVE PRODUCTION / INTEGRATE VITE FOR DEV
// ==========================================
...
async function start() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`MedID V4 server running on http://localhost:${PORT}`);
  });
}
